...
export async function GET(req,{params}) {
  ...
  try{
    ...
    populate: {
     ...
     return NextResponse.json({ status: 200, message: "Success", comments: blog.comments });
    ...
    console.log(params)
     ...
     try{
         ...
         if(decode instanceof NextResponse){
            ...
            if(!comment){
                return NextResponse.json({status:400,message:"All field are required"})
            }
        ...
        if(newComment){
               await Blog.updateOne({_id:blogId},{$push : {comments: newComment._id}})
        }
        return NextResponse.json({status:400,message: "comment is created succesfully",comment:newComment})
   ...
   console.log(commentId,blogId)
    ...
    try{
        ...
        if(!mongoose.Types.ObjectId.isValid(commentId) || !mongoose.Types.ObjectId.isValid(blogId)){
         ...
         if(deleteComment){
            await Blog.updateOne({_id:blogId}, {$pull : {comments: deleteComment._id}})
         }

         return NextResponse.json({status: 200 ,message:"Comment delete succesfully"})
    }catch(err){
...
export async function PUT(req,{params}) {
    ...
    try{
         ...
         return NextResponse.json({status: 200 ,message:"Comment update succesfully"})
