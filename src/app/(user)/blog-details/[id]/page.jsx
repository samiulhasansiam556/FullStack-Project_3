  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      ...
      <div className="flex items-center mb-6">
        <img 
          src={blog.blog.user.profileImage} 
          alt={blog.blog.user.name} 
          className="w-12 h-12 rounded-full object-cover mr-4"
        />
        <div>
          <p className="text-gray-600 text-sm">Blog created by</p>
          <h2 className="text-lg font-semibold">{blog.blog.user.name}</h2>
        </div>
      </div>
      ...
      <div className="prose max-w-none mb-8">
        <p className="whitespace-pre-line">{blog.blog.content}</p>
      </div>
      ...
      <div className="flex justify-between items-center border-t pt-4">
        ...
        <button
          ...
          className="text-blue-500 hover:text-blue-700 underline"
        >
          View Comments ({blog.blog.comments.length})
        </button>
      </div>
          ...
          <div className="bg-white p-6 rounded-lg max-w-md w-full max-h-[80vh] overflow-y-auto">
            ...
            {likes.length === 0 ? (
              ...
              <ul className="space-y-3">
                {likes.map(like => (
                  <li key={like._id} className="flex items-center space-x-3 p-2 hover:bg-gray-50 rounded">
                    <img 
